...
describe('Processor: base64', function() {
  /** @type {SchemaResolver} */
  let resolver;
  ...
  it('should accept valid base64 without padding', async function() {
    ...
    await compiled.validateValue('SGVsbG8'); // "Hello" in base64
  });
  ...
  it('should accept valid base64 with padding', async function() {
    ...
    await compiled.validateValue('SGVsbG8=');
    await compiled.validateValue('SGk=');
  });
  ...
  it('should reject invalid characters', async function() {
    ...
    await assert.rejects(() => compiled.validateValue('SGVs@G8='), ValidationError);
  ...
  it('should reject invalid padding', async function() {
    ...
    await assert.rejects(() => compiled.validateValue('SGVs=G8'), ValidationError); // padding not at end
    await assert.rejects(() => compiled.validateValue('SGVs==='), ValidationError); // too much padding
  });
  ...
  it('should reject base64 with invalid padding length', async function() {
    ...
    await assert.rejects(() => compiled.validateValue('A='), ValidationError); // length 2
    await assert.rejects(() => compiled.validateValue('AB='), ValidationError); // length 3
  });
});