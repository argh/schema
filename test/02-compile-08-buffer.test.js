...
import { NormalizeError, SerializeError, TransformError } from '../src/schema/schema-errors.js';
...
describe('Schema Compilation - Buffer Type', function() {
  ...
  describe('Buffer serialization', function() {
    ...
    it('should return undefined for non-Buffer values or throw in strict mode', async function() {
      ...
      await assert.rejects(
        async () => await compiled.serializeValue('not a buffer', undefined, undefined, {strict: true}),
        SerializeError
      );

      await assert.rejects(
        async () => await compiled.serializeValue(123, undefined, undefined, {strict: true}),
        SerializeError
      );
