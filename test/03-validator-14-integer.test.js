...
describe('Validator: integer', function() {
  ...
  it('should accept zero', async function() {
    ...
    const result = await compiled._validateValue('0');
  ...
  it('should accept large integers', async function() {
    ...
    const result = await compiled._validateValue('9007199254740991'); // MAX_SAFE_INTEGER
  ...
  it('should reject decimals', async function() {
    ...
    await assert.rejects(() => compiled._validateValue('3.14'), ValidationError);
    // Note: '42.0' parses to 42 which IS an integer, so it passes
    const result = await compiled._validateValue('42.0');
    assert.strictEqual(result, 42);
  });
