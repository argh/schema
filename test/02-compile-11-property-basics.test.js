...
describe('Schema Compilation - Property Basics', function() {
  ...
  describe('Property compilation independence', function() {
    ...
    it('should compile each property with its own base type', async function() {
      ...
      assert.strictEqual(await compiled.properties.text.normalize(42), '42');
      assert.strictEqual(await compiled.properties.count.normalize('42'), 42);
      assert.deepStrictEqual(await compiled.properties.items.normalize('a,b,c'), ['a', 'b', 'c']);
    });
