...
describe('Processor: $has-suffix', function() {
  /** @type {SchemaResolver} */
  let resolver;
  ...
  it('should pass through the input when it ends with the required suffix', async function() {
    const schema = await resolver.compile(new Schema('any').validator({'$has-suffix': {match: '.json'}}));
    ...
    // shorthand positional: {'$has-suffix': 'foo'} treated as {'$has-suffix': {match: 'foo'}}
    const shorthand = await resolver.compile(new Schema('any').validator({'$has-suffix': '.json'}));
