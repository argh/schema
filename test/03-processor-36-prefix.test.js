...
describe('Processor: prefix', function() {
  /** @type {SchemaResolver} */
  let resolver;
  ...
  it('should reject invalid configuration at compile time', async function() {
    ...
    await assert.rejects(() => resolver.compile(new Schema('any').validator('$prefix')), SchemaError);
    await assert.rejects(() => resolver.compile(new Schema('any').validator({$prefix: {}})), SchemaError);

    // unknown parameter
    await assert.rejects(
      () => resolver.compile(new Schema('any').validator({$prefix: {unexpected: 'foo'}})),
      SchemaError
    );
  });

  it('should pass through the input when it starts with the required prefix', async function() {
    const schema = await resolver.compile(new Schema('any').validator({$prefix: {match: 'http'}}));
    ...
    // shorthand positional: {$prefix: 'foo'} treated as {$prefix: {match: 'foo'}}
    const shorthand = await resolver.compile(new Schema('any').validator({$prefix: 'http'}));
