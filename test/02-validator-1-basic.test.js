...
describe('Validators', function() {
  ...
  describe('#validate()', function() {
    ...
    it('should pass with function validator that returns true', async function() {
      ...
      const result = await validators.validate('any value', validatorSpec);
      assert.notEqual(result, undefined);
      await assert.doesNotReject(async () => { await validators.validate('any value', validatorSpec); });
    });
    ...
    it('should fail with a function validator that returns an Error', async function() {
      ...
      await assert.rejects(async () => {
        await validators.validate('does not matter', validatorSpec);
      }, /Always fails/);
    });
    ...
    it('should pass with RegExp validator when value matches', async function() {
      ...
      const result = await validators.validate('12345', validatorSpec);
      assert.notEqual(result, undefined);
      await assert.doesNotReject(async () => { await validators.validate('12345', validatorSpec); });
    });

    it('should fail with RegExp validator when value does not match', async function() {
      const validatorSpec = /^\d+$/;
      await assert.rejects(async () => {
        await validators.validate('abc123', validatorSpec);
    ...
    it('should pass with string pattern validator when value matches', async function() {
      ...
      const result = await validators.validate('abc123', validatorSpec);
      assert.notEqual(result, undefined);
      await assert.doesNotReject(async () => { await validators.validate('abc123', validatorSpec); });
    });

    it('should fail with string pattern validator when value does not match', async function() {
      const validatorSpec = '/^\\w+$/';
      await assert.rejects(async () => {
        await validators.validate('abc 123', validatorSpec);
      }, /Value does not match pattern/);
    });
    ...
    it('should pass with string exact match validator when value matches', async function() {
      ...
      const result = await validators.validate('exact-match', validatorSpec);
      assert.notEqual(result, undefined);
      await assert.doesNotReject(async () => { await validators.validate('exact-match', validatorSpec); });
    });

    it('should fail with string exact match validator when value does not match', async function() {
      const validatorSpec = 'exact-match';
      await assert.rejects(async () => {
        await validators.validate('not-a-match', validatorSpec);
    ...
    it('should pass with built-in email validator for valid email', async function() {
      ...
      const result = await validators.validate('test@example.com', validatorSpec);
      assert.notEqual(result, undefined);
      await assert.doesNotReject(async () => { await validators.validate('test@example.com', validatorSpec); });
    });

    it('should fail with built-in email validator for invalid email', async function() {
      const validatorSpec = '$email';
      await assert.rejects(async () => {
        await validators.validate('not-an-email', validatorSpec);
      }, /Invalid email format/);
    });

    it('should pass with built-in hostname validator for valid hostname', async function() {
      const validatorSpec = '$hostname';
      const result = await validators.validate('example.com', validatorSpec);
    ...
    it('should fail with built-in hostname validator for invalid hostname', async function() {
      const validatorSpec = '$hostname';
      await assert.rejects(async () => {
        await validators.validate('invalid..hostname', validatorSpec);
      }, /Invalid hostname format/);
    });

    it('should pass with built-in url validator for valid URL', async function() {
      const validatorSpec = '$url';
      const result = await validators.validate('https://example.com', validatorSpec);
    ...
    it('should fail with built-in url validator for invalid URL', async function() {
      const validatorSpec = '$url';
      await assert.rejects(async () => {
        await validators.validate('not-a-url', validatorSpec);
      }, /Invalid URL format/);
    });
    ...
    it('should pass with $length validator when length is within range', async function() {
      ...
      const result = await validators.validate('test', validatorSpec);
    ...
    it('should fail with $length validator when length is below minimum', async function() {
      ...
      await assert.rejects(async () => {
        await validators.validate('test', validatorSpec);
    ...
    it('should fail with $length validator when length exceeds maximum', async function() {
      ...
      await assert.rejects(async () => {
        await validators.validate('test123', validatorSpec);
    ...
    it('should pass with $length validator with exact match', async function() {
      ...
      const result = await validators.validate('test', validatorSpec);
      assert.notEqual(result, undefined);
      await assert.doesNotReject(async () => { await validators.validate('test', validatorSpec); });
    });
    ...
    it('should pass with $range validator when value is within range', async function() {
      ...
      const result = await validators.validate(7, validatorSpec);
      assert.notEqual(result, undefined);
      await assert.doesNotReject(async () => { await validators.validate(7, validatorSpec); });
    });

    it('should fail with $range validator when value is below minimum', async function() {
      const validatorSpec = { $range: { min: 5 } };
      await assert.rejects(async () => {
        await validators.validate(3, validatorSpec);
    ...
    it('should fail with $range validator when value exceeds maximum', async function() {
      ...
      await assert.rejects(async () => {
        await validators.validate(15, validatorSpec);
      }, /Value must be at most/);
    });

    // Test with $oneof validator
    it('should pass with $in validator when value is in the list', async function() {
      const validatorSpec = { $in: ['option1', 'option2', 'option3'] };
      const result = await validators.validate('option2', validatorSpec);
      assert.notEqual(result, undefined);
      await assert.doesNotReject(async () => { await validators.validate('option2', validatorSpec); });
    });

    it('should fail with $in validator when value is not in the list', async function() {
      const validatorSpec = { $in: ['option1', 'option2', 'option3'] };
      await assert.rejects(async () => {
        await validators.validate('option4', validatorSpec);
    ...
    it('should fail with $and validator when one condition fails', async function() {
      const validatorSpec = { $and: [/^\d+$/, { $length: { min: 3, max: 5 } }] };
      await assert.rejects(async () => {
        await validators.validate('123456', validatorSpec);
      }, /Length must be at most/);
    });
    ...
    it('should pass with $not validator when condition fails', async function() {
      ...
      const result = await validators.validate('abc', validatorSpec);
      assert.notEqual(result, undefined);
      await assert.doesNotReject(async () => { await validators.validate('abc', validatorSpec); });
    });

    it('should fail with $not validator when condition passes', async function() {
      const validatorSpec = { $not: /^\d+$/ };
      await assert.rejects(async () => {
        await validators.validate('12345', validatorSpec);
    ...
    it('should pass with positive validator for positive number', async function() {
      ...
      const result = await validators.validate(42, validatorSpec);
      assert.notEqual(result, undefined);
      await assert.doesNotReject(async () => { await validators.validate(42, validatorSpec); });
    });

    it('should fail with positive validator for negative number', async function() {
      const validatorSpec = '$positive';
      await assert.rejects(async () => {
        await validators.validate(-5, validatorSpec);
      }, /Must be a positive number/);
    });

    it('should pass with negative validator for negative number', async function() {
      const validatorSpec = '$negative';
      const result = await validators.validate(-10, validatorSpec);
      assert.notEqual(result, undefined);
      await assert.doesNotReject(async () => { await validators.validate(-10, validatorSpec); });
    });

    it('should fail with negative validator for positive number', async function() {
      const validatorSpec = '$negative';
      await assert.rejects(async () => {
        await validators.validate(10, validatorSpec);
    ...
    it('should fail with integer validator for float', async function() {
      const validatorSpec = '$integer';
      await assert.rejects(async () => {
        await validators.validate(3.14, validatorSpec);
      }, /Must be an integer/);
    });
    ...
    it('should pass when validatorSpec is undefined', async function() {
      ...
      const result = await validators.validate(value, undefined);
      assert.equal(result, value);
      await assert.doesNotReject(async () => { await validators.validate(value, undefined); });
    });

    it('should pass when validatorSpec is null', async function() {
      const value = 'any value';
      const result = await validators.validate(value, null);
      assert.equal(result, value);
      await assert.doesNotReject(async () => { await validators.validate(value, null); });
    });
  });
});
